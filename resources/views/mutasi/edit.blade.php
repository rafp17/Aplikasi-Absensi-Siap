            ...
            <div>
                ...
                <select name="pegawai_id" class="w-full p-3 rounded bg-gray-700 border border-gray-600 text-white focus:border-red-600 focus:ring-1 focus:ring-red-600 outline-none transition">
                    @foreach($pegawai as $p)
                        <option value="{{ $p->id }}" {{ $mutasi->pegawai_id == $p->id ? 'selected' : '' }}>
                            {{ $p->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="jenis_mutasi" class="w-full p-3 rounded bg-gray-700 border border-gray-600 text-white focus:border-red-600 outline-none">
                    @foreach(['Kenaikan Pangkat', 'Masuk', 'Pindah Antar Instansi', 'Keluar'] as $jenis)
                        <option value="{{ $jenis }}" {{ $mutasi->jenis_mutasi == $jenis ? 'selected' : '' }}>
                            {{ $jenis }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="tanggal" 
                           value="{{ \Carbon\Carbon::parse($mutasi->tanggal)->format('Y-m-d') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="tujuan_pengiriman" class="w-full p-3 rounded bg-gray-700 border border-gray-600 text-white focus:border-red-600 outline-none">
                        ...
                        <option value="BKPSDM" {{ (old('tujuan_pengiriman', $mutasi->tujuan_pengiriman) == 'BKPSDM') ? 'selected' : '' }}>
                            BKPSDM
                        </option>
                        
                        <option value="BKPSDM-SEKDA" {{ (old('tujuan_pengiriman', $mutasi->tujuan_pengiriman) == 'BKPSDM-SEKDA') ? 'selected' : '' }}>
                            BKPSDM-SEKDA
                        </option>
                        
                        <option value="BKPSDM-SEKDA-BUPATI" {{ (old('tujuan_pengiriman', $mutasi->tujuan_pengiriman) == 'BKPSDM-SEKDA-BUPATI') ? 'selected' : '' }}>
                            BKPSDM-SEKDA-BUPATI
                        </option>
                    </select>
                </div>
            ...
            <div class="p-4 bg-gray-900 rounded border border-gray-700">
                ...
                @if($mutasi->file_sk)
                    <div class="mt-2 text-sm text-blue-400 flex items-center gap-2">
                        ...
                        <span>File saat ini: Tersedia ({{ basename($mutasi->file_sk) }})</span>
                    </div>
                @endif
            </div>
